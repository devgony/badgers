...
### 8.1 PR comment (PR marker 기반 upsert)

```text
1. workflow는 PR별 concurrency + cancel-in-progress로 직렬화한다.
2. 현재 PR head SHA와 분석한 head SHA가 다르면 stale run으로 판단하고 게시하지 않는다.
3. GitHub Actions 앱이 작성한 issue comment에서 marker를 검색한다: <!-- badgers-report:{owner}/{repo}:{pr_number} -->
4. 있으면 PATCH (latest result update), 없으면 POST (create)
```

GitHub issue-comment API에는 atomic compare-and-swap이 없으므로 workflow
concurrency가 설정되지 않은 호출자의 갱신은 best-effort이다.

comment 형식:

```markdown
<!-- badgers-report:jubilee-works/timetree-planner-server:123 -->
